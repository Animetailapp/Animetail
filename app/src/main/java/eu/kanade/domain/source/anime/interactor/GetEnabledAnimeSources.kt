...
import tachiyomi.domain.source.anime.model.AnimePin
import tachiyomi.domain.source.anime.model.AnimePins
    ...
    fun subscribe(): Flow<List<AnimeSource>> {
        return combine(
            ...
            sources
                ...
                .flatMap {
                    val flag = if ("${it.id}" in pinnedSourceIds) AnimePins.pinned else AnimePins.unpinned
                    val source = it.copy(pin = flag)
                    val toFlatten = mutableListOf(source)
                    if (source.id == lastUsedSource) {
                        toFlatten.add(source.copy(isUsedLast = true, pin = source.pin - AnimePin.Actual))
                    }
                    toFlatten
                }
        }
